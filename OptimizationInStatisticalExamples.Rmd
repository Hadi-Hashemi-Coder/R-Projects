...
Name: Hadi Hashemi Nejad

Email: mhashemineja@wisc.edu
...
```{r}
plot(area$land,area$farm,main="farm vs land",xlab="land",ylab="farm")

#THIS IS WRONG
rlm = lm(farm~land, data=area)
abline(rlm$coefficients,col="limegreen")

Tukey = function(par,k=19000){
  B0 = par[1]
  B1 = par[2]
  sum.f=0
  for(row in 1:nrow(area)){
    t = area[row,3]-B0-B1*area[row,2]
    if(abs(t) <= k){
      sum.f = sum.f + t**2
    } else if(abs(t) > k){
      sum.f = sum.f + 2*k*abs(t)-k**2
    }
  }
  return((1/nrow(area))*sum.f)
}
Tukey.prime = function(par,k=19000){
  B0 = par[1]
  B1 = par[2]
  sum.f.B0.prime=0
  sum.f.B1.prime=0
  for(row in 1:nrow(area)){
    t = area[row,3]-B0-B1*area[row,2]
    if(abs(t) <= k){
      sum.f.B0.prime = sum.f.B0.prime + 2*t
      sum.f.B1.prime = sum.f.B1.prime + 2*t*area[row,2]
    } else if(abs(t) > k){
      sum.f.B0.prime = sum.f.B0.prime + 2*k*sign(t)
      sum.f.B1.prime = sum.f.B1.prime + 2*k*sign(t)*area[row,2]
    }
  }
  return(c(-(1/nrow(area))*sum.f.B0.prime,-(1/nrow(area))*sum.f.B1.prime))
}

#Nelder-Mead method
n.m = optim(par=c(0,0), fn = Tukey, method = "Nelder-Mead")
abline(coef=n.m$par,col="navy")

#BFGS
bfgs = optim(par=c(0,0), fn = Tukey, gr = Tukey.prime, method = "BFGS")
abline(coef=bfgs$par,col="black")

#BFGS
cg = optim(par=c(0,0), fn = Tukey, gr = Tukey.prime, method = "CG")
abline(coef=cg$par,col="coral",lty="dotted")

legend(0, 200000, legend=c("least squares regression method", "Nelder-Mead method", "BFGS method", "CG method"), 
       fill = c("limegreen","navy","black","coral")
)
```

Another view of the different methods

```{r}
par(mfrow=c(2,2))
plot(area$land,area$farm,main="least squares regression method",xlab="land",ylab="farm")
abline(rlm$coefficients,col="limegreen")
plot(area$land,area$farm,main="Nelder-Mead method",xlab="land",ylab="farm")
abline(coef=n.m$par,col="navy")
plot(area$land,area$farm,main="BFGS method",xlab="land",ylab="farm")
abline(coef=bfgs$par,col="black")
plot(area$land,area$farm,main="CG method",xlab="land",ylab="farm")
abline(coef=cg$par,col="coral",lty="dotted")
```
...
```{r}
smallest.Tukey = min(c(n.m$value,bfgs$value,cg$value))
print(paste0("The smallest Tukey value was found by BFGS method at: ",smallest.Tukey))

p = function(t, k=19000){
  return(ifelse(abs(t) <= k,t**2,2*abs(t)*k-k**2))
}

curve(p, from = -100000, to = 100000, main="plot of p(t)")
abline(v=c(-19000,19000), col="red",lty="dotted")

print("Outliers have less of an influence on the robust line since outliers that have a abs(t) greater than k  will have a p(t) smaller than what it would be in simple least squares regression line. When minimizing the cost of the difference between line and points, outliers will have less influence on the cost. And so, not be dragged up or down, more than it should be, by the outlier,")
```
...
```{r}
mean.forecast.error = function(B){
  sum.forecast.error = 0
  forecasts = c(nhtemp[1])
  for(year in 2:length(nhtemp)){
    Y = B*nhtemp[year-1]+(1-B)*forecasts[year-1]
    forecasts = c(forecasts, Y)
    sum.forecast.error = sum.forecast.error + (nhtemp[year]-forecasts[year])**2
  }
  return((1/length(nhtemp))*sum.forecast.error)
}

B.optimal=optimize(mean.forecast.error,lower=0,upper=1)
```
...
```{r}
# m=HoltWinters(nhtemp, beta = FALSE, gamma = FALSE)
# plot(m)

forecasted = function(B.optimal){
  forecasts = c(nhtemp[1])
  for(year in 2:length(nhtemp)){
    Y = B.optimal*nhtemp[year-1]+(1-B.optimal)*forecasts[year-1]
    forecasts = c(forecasts, Y)
  }
  return(ts(forecasts,start=1912,end=1971))
}
plot(nhtemp)
lines(forecasted(B.optimal$minimum),col="red")
```
...
plot(nhtemp)
lines(forecasted(B.optimal$minimum),col="red")
lines(forecasted(0.1),col="blue")
lines(forecasted(0.9),col="orange")
legend(1910,54,legend=c("opimized B","B=0.1","B=0.9"),fill=c("red","blue","orange"))
```
...
```{r}
log.likelihood = function(par,data=mtcars$mpg){
  sum.total = 0
  u=par[1]
  s=par[2]
  for(i in 1:length(data)){
    sum.total = sum.total + (data[i]-u)**2
  }
  return(length(data)*log(1/(s*(2*pi)**(1/2)))-(1/(2*s**2))*sum.total)
}
o=optim(par=c(3,9),fn=log.likelihood,control=list(fnscale = -1))
print(paste0("mu of ",o$par[1]," and sigma of ",o$par[2]," maximize the loglikelihood for mtcars"))
```
...
```{r}
log.likelihood.m = function(par,data=menarche.cases){
  B0 = par[1]
  B1 = par[2]
  total.1=0
  total.2=0
  for(i in 1:nrow(data)){
    total.1 = total.1 + log(1+exp(B0+B1*data[i,1]))
    total.2 = total.2 + data[i,2]*(B0+B1*data[i,1])
  }
  return(-total.1+total.2)
}

o.m = optim(par=c(10,1),fn=log.likelihood.m,control=list(fnscale = -1))

c=function(x,B=o.m$par){
  return(1/(1+exp(-(B[1]+B[2]*x))))
}

plot(x=jitter(menarche.cases[,1]),y=menarche.cases[,2],main="age vs menarche",xlab="age",ylab="percent chance of first menarche")
points(x=menarche[,1],y=menarche[,3]/menarche[,2],col="red")
curve(c,from=8,to=25,add = TRUE)
```
