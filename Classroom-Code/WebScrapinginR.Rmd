...
```{r}
land = readHTMLTable(readLines("https://simple.wikipedia.org/wiki/List_of_U.S._states_by_area"), stringsAsFactors = FALSE)
land = land[[1]][3:52,c(1,6)]
land[,2]=sub(pattern=",",replacement="",x=land[,2])
```

Use R to get farm areas of states from "farm.csv".
```{r}
farm = read.csv(file="farm.csv",header=TRUE)
```
...
```{r}
plot(area[,3], area[,2], main="farm area vs land area",xlab="land area", ylab="farm area")
```
...
```{r}
area[,3]=as.numeric(area[,3])
plot(x=area$land,y=area$farm, main="farm area vs land area",xlab="land area", ylab="farm area")
model.1=lm(farm ~ land,data=area)
abline(model.1)

area.without.alaska = area[c(1,3:50),]
plot(x=area.without.alaska$land,y=area.without.alaska$farm, main="farm area vs land area without Alaska",xlab="land area", ylab="farm area")
model.2=lm(farm~land,data=area.without.alaska)
abline(model.2)
```
...
for(i in 1:50){
  area.new = area[-i,]
  model = lm(farm~land,data=area.new)
  p = predict(model, data.frame(land = area[i,3]))
  r.jack = c(r.jack, (area[i,2]-p[[1]]))
}
```
...
```{r}
lines = readLines(curl::curl("http://www.imdb.com/chart/top"))
movie.links = unique(grep(x=lines,pattern="^<a\\shref=\"/title/",value=TRUE))
movie.links = sub(pattern=".*\"(/title/\\w\\w\\d{7,}).*\"",x=movie.links,replacement="https://www.imdb.com\\1/fullcredits")
producers = list()
for(movie.link in movie.links){
  lines.movie = readLines(movie.link)
  title = grep(pattern="^\\s*<title>",x=lines.movie,value=TRUE)
  title = sub(pattern=".*<title>(.*)\\s-\\sFull\\sCast.*",replacement="\\1",x=title)
  print(paste("trying: ",title))
  
  table.start.locations = grep(x=lines.movie,pattern="^\\s*<table\\sclass=.*")
  affilates.line = table.start.locations - 1
  produced.by.line = grep(x=lines.movie[affilates.line],pattern="Produced by")
  
  if(length(produced.by.line)!=0){
      producers[[title]] = readHTMLTable(lines.movie[table.start.locations[produced.by.line]:(table.start.locations[produced.by.line+1]-1)])[[1]][,1]
  }
}
producers = unlist(producers)
producers = table(producers)
print(tail(sort(producers),5))
```
